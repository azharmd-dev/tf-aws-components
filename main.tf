...
#Stoping instance to take an image
...
resource "aws_lb_target_group" "main" {
  ...
  protocol = "HTTP"
  vpc_id   = local.vpc_id
  deregistration_delay = 60 #Waiting period for deleting the instance
...
resource "aws_launch_template" "main" {
  name = "${local.common_name_suffix}-${var.component}-lt"
...
resource "aws_autoscaling_group" "main" {
  ...
  vpc_zone_identifier       = local.private_subnet_ids #It will use two subnets
